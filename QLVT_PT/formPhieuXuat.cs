...
namespace QLVT_PT
{
    public partial class formPhieuXuat : Form
    {
        ...
        private void btnGhi_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            /*if(them == true)
            {
                if (MessageBox.Show("Bạn có muốn thêm các vật tư cho Phiếu Xuất?", "Xác nhận", MessageBoxButtons.OKCancel) == DialogResult.OK)
                {
                    addCTPX();
                }
            } */
        ...
        private void btnGhiVT_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    string statement = "EXEC sp_KiemTraSoLuongCTPX '" + txtMAVT.Text + "', " + soluong;
                    Program.myReader = null;
                    Program.myReader = Program.ExecSqlDataReader(statement);
                    if (Program.myReader == null)
                        return;
                    ...
                    {
                        MessageBox.Show("Số lượng vật tư này lớn hơn so với trong kho, vui lòng chỉnh lại", "", MessageBoxButtons.OK);
                        Program.myReader.Close();
                        return;
                    }
                    Program.myReader.Close();
            ...
            try
            {
                if (ThemVT == true)
                {
                    UpdateSoLuongTonVattu(txtMAVT.Text, Convert.ToInt32(txtSOLUONG.Text), false);
                    this.vattuTableAdapter.Connection.ConnectionString = Program.connstr;
